...
def process_new_datafiles(datafiles, ignore):
    ...
    try:
        ...
        datafiles_done = df['datafile'].tolist()
    except KeyError:
        datafiles_done = []

    # select source datafiles
    #datafiles_done = [name[4:] for name in datafiles_done0]
    datafiles = mylib.pick_datafiles(datafiles, ignore + datafiles_done)
    print("N of datafiles:", len(datafiles))

    # process source datafiles
    fig = plt.figure()
    for dataf in datafiles:
        ...
        if not exist:
            ...
            while mode == "r":
                ...
                plt.subplot(121)
                plothyst(data["B"], data["Icp"], "o")
                plothyst(data["B"], data["Icn"], "o")

                # subplot 2: a sample IV
                data0, md0  = datafile.load_data(dataf)
                ind         = 0
                data0['I'], data0['V'] = data0['IV'][:, 0], data0['IV'][:, 1]
                IV = datafile.deserialize_data(data0, ['B'], ['I', 'V'])
                x           = IV["I"][ind]
                y           = IV["V"][ind]
                plt.subplot(122)
                plt.plot(x, y, "o")
            ...
            if mode == "c":
                ...
                while mode == "r":
                    ...
                    func, x             = md["fitfunc_nosave"], md["fitx_nosave"]
...
def show_BIc():
    ...
    for k in df.index:
        ...
        fitcurve            = build_curve(func, data["B"], *popt)
        plt.plot(fitcurve[0], fitcurve[1], "-", lw=3)
...
def show_thickness_dependence(dbs=["results.txt"], devspecs=[{}]):
    """Extract, plot, and save Ic(d) and Bcen(d).
...
def plot_thk_dep(dbs=['result_thickness']):
    """Simply plot results from the db files.

    Arguments:
        dbs: list of thickness dependence datafile
    """
    ...
    for db in dbs:
        df = mylib.load_db(db)
        plt.subplot(211)
        plt.plot(df.d, df.Vcp, "o")
        plt.subplot(212)
        plt.plot(df.d, df.Hcen, "o")
